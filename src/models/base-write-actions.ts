...
export abstract class BaseWriteActions
  extends FindFailActions
  implements BaseWriteActionsInterface
{
  /**
  ...
  public destroy(): Promise<boolean> {
    ...
    if (this.builder.length === 0 && this.indexBuilder === null) {
      throw new Error(
        'Deletion without builder is not allowed. Call truncate if you want to delete all records.',
      );
    }
    ...
    if (
      ...
      this.primaryId === this.indexBuilder.index
    ) {
   ...
   * @param mergeDeep
   */
  public update(data: any, mergeDeep = true): Promise<number> {
    const tables = [this.table.name];
    const transaction = this.getTransaction(tables, TransactionModes.Write);
    const objectStore = transaction.objectStore(this.table.name);
    ...
    return new Promise<number>((resolve, reject) => {
      ...
      request.addEventListener<'success'>('success', async (event) => {
        ...
        if (cursor) {
          ...
          updatePromises.push(
            this.save(cursor.value[this.primaryId], data, mergeDeep),
          );
          totalUpdatedRecords++;
          cursor.continue();
        }
   ...
   * @throws NotFound
   */
  public async save(id: any, data: any, mergeDeep = true): Promise<boolean> {
    const tables = [this.table.name];
    const transaction = this.getTransaction(tables, TransactionModes.Write);
